...
// admins: [{ username, password }] — up to a few organizers who may open the statistics page.
function createApi(db, { admins = [], adminUsername = '', adminPassword = '', sessionSecret } = {}) {
  const accounts = (admins.length ? admins : [{ username: adminUsername, password: adminPassword }])
  ...
  const secret =
    ...
    crypto
      .createHash('sha256')
      .update('rsvp-session:' + accounts.map((a) => `${a.username}:${a.password}`).join('|'))
      .digest('hex');
  const sign = (value) => crypto.createHmac('sha256', secret).update(value).digest('base64url');

  // Session cookie = "<expires>.<username>.<signature>". Removing an account ends its sessions.
  function sessionUser(cookieHeader) {
    const parts = (parseCookies(cookieHeader).admin_session || '').split('.');
    if (parts.length !== 3) return null;
    const [expires, user, sig] = parts;
    if (!safeEqual(sig, sign(`${expires}.${user}`)) || !(Number(expires) > Date.now())) return null;
    const username = Buffer.from(user, 'base64url').toString('utf8');
    return accounts.some((a) => a.username === username) ? username : null;
  }
  ...
  // Rate limits. Counters live in storage when it supports them (so they hold across Netlify's
  // short-lived function instances), otherwise in memory.
  const memoryCounters = new Map();
  const counters = {
    get: (key) => (db.getCounter ? db.getCounter(key) : memoryCounters.get(key)),
    set: (key, rec) => (db.setCounter ? db.setCounter(key, rec) : memoryCounters.set(key, rec)),
  };
  async function isLimited(bucket, ip, limit, windowMs) {
    const rec = await counters.get(`${bucket}:${ip}`);
    return !!rec && Date.now() - rec.first <= windowMs && rec.count >= limit;
  }
  async function count(bucket, ip, windowMs) {
    const key = `${bucket}:${ip}`;
    const rec = await counters.get(key);
    const next = !rec || Date.now() - rec.first > windowMs ? { first: Date.now(), count: 1 } : { ...rec, count: rec.count + 1 };
    await counters.set(key, next);
  }
  const LOGIN_LIMIT = 8;
  const LOGIN_WINDOW = 15 * 60 * 1000;
  const RSVP_LIMIT = 40;
  const RSVP_WINDOW = 60 * 60 * 1000;
  ...
  async function route({ method, pathname, searchParams, headers, body, ip, secure }) {
    ...
    if (pathname === '/api/rsvp' && method === 'POST') {
      if (await isLimited('rsvp', ip, RSVP_LIMIT, RSVP_WINDOW)) {
        return json(429, { error: 'נשלחו יותר מדי תשובות מהחיבור הזה. נסו שוב מאוחר יותר.' });
      }
      ...
      if (input.replaceId !== undefined) {
        ...
        await count('rsvp', ip, RSVP_WINDOW);
      ...
      if (input.confirmNew !== true) {
        ...
        // Only the name and date are shared back — not what that person answered.
        if (match) return json(409, { match: { id: match.id, firstName: match.firstName, lastName: match.lastName, createdAt: match.createdAt } });
      }
    ...
    if (pathname === '/api/admin/login' && method === 'POST') {
      ...
      if (await isLimited('login', ip, LOGIN_LIMIT, LOGIN_WINDOW)) {
        return json(429, { error: 'יותר מדי ניסיונות. נסו שוב בעוד רבע שעה.' });
      }
      const { username, password } = parseBody(body);
      // Compare against every account without stopping early, so timing reveals nothing.
      let matched = null;
      for (const a of accounts) {
        const ok =
          (typeof username === 'string' && safeEqual(username.trim(), a.username)) &
          (typeof password === 'string' && safeEqual(password, a.password));
        if (ok && !matched) matched = a.username;
      }
      if (!matched) {
        await count('login', ip, LOGIN_WINDOW);
        return json(401, { error: 'שם משתמש או סיסמה שגויים' });
      }
      const expires = String(Date.now() + SESSION_TTL_MS);
      const user = Buffer.from(matched, 'utf8').toString('base64url');
      const token = `${expires}.${user}.${sign(`${expires}.${user}`)}`;
      return json(200, { ok: true, username: matched }, { 'Set-Cookie': sessionCookie(token, SESSION_TTL_MS / 1000, secure) });
    }
    ...
    const adminUser = sessionUser(headers.cookie);
    if (!adminUser) return json(401, { error: 'נדרשת התחברות' });

    if (pathname === '/api/admin/responses' && method === 'GET') {
      const responses = (await db.listResponses()).map(publicResponse);
      return json(200, { summary: summarize(responses), responses, event: await getEvent(), username: adminUser });
    }
  ...
  return async function handle(req) {
    try {
      ...
      console.error(err);
      // The short code (e.g. "BlobsInternalError") helps diagnose problems without exposing details.
      const code = String((err && err.name) || 'Error').replace(/[^A-Za-z0-9_]/g, '').slice(0, 40);
      return json(500, { error: `אירעה שגיאה בשרת. נסו שוב. (קוד: ${code})`, code });
    }
  };
}
