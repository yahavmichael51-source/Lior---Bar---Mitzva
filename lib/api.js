...
function createApi(db, { adminPassword = '', sessionSecret } = {}) {
  const secret =
    sessionSecret || crypto.createHash('sha256').update('rsvp-session:' + adminPassword).digest('hex');
  ...
  async function route({ method, pathname, searchParams, headers, body, ip, secure }) {
    ...
    if (pathname === '/api/rsvp' && method === 'POST') {
      ...
      return json(201, { ok: true, response: await db.addResponse(value) });
    }

    if (pathname === '/api/admin/login' && method === 'POST') {
      if (!adminPassword) return json(503, { error: 'לא הוגדרה סיסמת מנהל בשרת (ADMIN_PASSWORD)' });
      if (tooManyAttempts(ip)) return json(429, { error: 'יותר מדי ניסיונות. נסו שוב בעוד רבע שעה.' });
      const { password } = parseBody(body);
    ...
    if (m && method === 'PUT') {
      ...
      return updated ? json(200, { ok: true, response: updated }) : json(404, { error: 'התשובה לא נמצאה' });
    }
    if (m && method === 'DELETE') {
...
module.exports = { createApi, validateResponse, summarize, byNewest, DEFAULT_EVENT, MAX_BODY };
