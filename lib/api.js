  ...
  async function route({ method, pathname, searchParams, headers, body, ip, secure }) {
    ...
    if (pathname === '/api/rsvp' && method === 'POST') {
      ...
      if (input.replaceId !== undefined) {
        ...
        const updated = await db.updateResponse(existing.id, value, { editTokenHash: hashToken(editToken) });
        return updated
          ? json(200, { ok: true, replaced: true, response: publicResponse(updated), editToken })
      ...
      if (input.confirmNew !== true) {
        ...
        if (match) return json(409, { match: publicResponse(match) });
      }

      const saved = await db.addResponse({ ...value, editTokenHash: hashToken(editToken) });
      return json(201, { ok: true, response: publicResponse(saved), editToken });
    }
    ...
    if (own && (method === 'GET' || method === 'PUT')) {
      ...
      if (method === 'GET') return json(200, { response: publicResponse(existing) });
      const { value, error } = validateResponse(parseBody(body));
      if (error) return json(400, { error });
      const updated = await db.updateResponse(own[1], value);
    ...
    if (m && method === 'PUT') {
      ...
      if (error) return json(400, { error });
      const updated = await db.updateResponse(m[1], value);
      return updated ? json(200, { ok: true, response: publicResponse(updated) }) : json(404, { error: 'התשובה לא נמצאה' });
    }
    if (m && method === 'DELETE') {
    ...
    if (pathname === '/api/admin/export.csv' && method === 'GET') {
      ...
      const valid = STATUSES.includes(filter);
      let rows = (await db.listResponses()).map(publicResponse);
      if (valid) rows = rows.filter((r) => r.status === filter);
      const lines = [['שם פרטי', 'שם משפחה', 'תשובה', 'מספר אנשים', 'שעת שליחה', 'עודכן לאחרונה'].map(csvCell).join(',')];
      for (const r of rows) {
        lines.push(
          [
            r.firstName,
