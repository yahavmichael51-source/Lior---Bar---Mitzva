...
async function createPostgres(url) {
  ...
  await pool.query(`
    ...
    ALTER TABLE responses ADD COLUMN IF NOT EXISTS history TEXT NOT NULL DEFAULT '[]';
  `);
  return {
    kind: 'postgres',
    ...
    async updateResponse(id, { firstName, lastName, status, people }, { editTokenHash, history } = {}) {
      if (!isId(id)) return null;
      const { rows } = await pool.query(
        `UPDATE responses SET first_name=$1, last_name=$2, status=$3, people=$4, updated_at=now(),
           edit_token_hash=COALESCE($6, edit_token_hash), history=COALESCE($7, history) WHERE id=$5 RETURNING *`,
        [firstName, lastName, status, people, id, editTokenHash || null, history ? JSON.stringify(history) : null]
      );
      return rows[0] ? rowToResponse(rows[0]) : null;
    },
...
function createSqlite(file) {
  ...
  if (!columns.has('history')) db.exec("ALTER TABLE responses ADD COLUMN history TEXT NOT NULL DEFAULT '[]'");
  ...
  return {
    ...
    async updateResponse(id, { firstName, lastName, status, people }, { editTokenHash, history } = {}) {
      ...
      const row = db
        .prepare(
          `UPDATE responses SET first_name=?, last_name=?, status=?, people=?, updated_at=?,
             edit_token_hash=COALESCE(?, edit_token_hash), history=COALESCE(?, history) WHERE id=? RETURNING *`
        )
        .get(
          firstName,
          lastName,
