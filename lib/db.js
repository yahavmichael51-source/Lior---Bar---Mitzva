...
async function createPostgres(url) {
  ...
  return {
    ...
    async updateResponse(id, { firstName, lastName, status, people }, { editTokenHash } = {}) {
      if (!isId(id)) return null;
      const { rows } = await pool.query(
        `UPDATE responses SET first_name=$1, last_name=$2, status=$3, people=$4, updated_at=now(),
           edit_token_hash=COALESCE($6, edit_token_hash) WHERE id=$5 RETURNING *`,
        [firstName, lastName, status, people, id, editTokenHash || null]
      );
      return rows[0] ? rowToResponse(rows[0]) : null;
    },
      ...
      const row = db
        .prepare(
          `UPDATE responses SET first_name=?, last_name=?, status=?, people=?, updated_at=?,
             edit_token_hash=COALESCE(?, edit_token_hash) WHERE id=? RETURNING *`
        )
        .get(firstName, lastName, status, people, now(), editTokenHash || null, Number(id));
      return row ? rowToResponse(row) : null;
    },
    async deleteResponse(id) {
